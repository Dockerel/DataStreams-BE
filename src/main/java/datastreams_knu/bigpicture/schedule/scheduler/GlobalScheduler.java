...
public class GlobalScheduler {
    ...
    // 한국 / 미국 금리 크롤링
    @Scheduled(cron = "0 30 1 * * MON", zone = "Asia/Seoul") // 월요일 새벽 2시
    public void runInterestCrawling() {
        schedulerService.interestCrawling();
    }

    // 기본 경제 뉴스 크롤링
    @Scheduled(cron = "0 0 2 * * MON", zone = "Asia/Seoul") // 월요일 새벽 3시
    public void runGeneralNewsCrawling() {
        schedulerService.generalNewsCrawling();
    }

    // 주가 + 관련 뉴스 크롤링
    @Scheduled(cron = "0 30 2 * * MON", zone = "Asia/Seoul") // 월요일 새벽 4시
